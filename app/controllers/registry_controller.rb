...
class RegistryController < ApplicationController
  ...
  def index
      ...
      @worksets = registry.list_worksets(current_user.name)
  ...
  def load
      ...
      registry = Registry.new
      ids = registry.get_workset_volumes(current_user.name, workset_name)
      ...
      respond_to do |format|
         ...
         format.html { redirect_to "/folder", :notice =>  I18n.t('blacklight.registry.load.success', :name => workset_name) }
  ...
  def export
      ...
      if params[:ws] != nil
          ...
          registry = Registry.new
          registry.update_workset(current_user.name, name, desc, avail, tags)
          registry.update_volumes(current_user.name, name, ids)
          ...
          registry = Registry.new
          registry.create_workset(current_user.name, name, desc, avail, tags, ids)
